...
export default function PublicContractorProfilePage() {
  ...
  useEffect(() => {
    if (contractorSlug) {
      ...
      setError('No contractor slug provided')
      setLoading(false)
    }
  }, [contractorSlug])
  ...
  const fetchContractorProfile = async () => {
    try {
      ...
      const { data: contractorsWithSlugs, error: slugsError } = await supabase
      ...
      try {
        ...
        contractorError = queryError
        contractorData = null
      }

      if (contractorError) {
        console.error('Error fetching contractor by slug:', contractorError)
        console.error('Contractor slug:', contractorSlug)
        console.error('Error details:', JSON.stringify(contractorError, null, 2))
        console.error('Error code:', (contractorError as any)?.code)
        console.error('Error message:', (contractorError as any)?.message)
        console.error('Error hint:', (contractorError as any)?.hint)
        console.error('Error details (stringified):', (contractorError as any)?.details)
        console.error('Error type:', typeof contractorError)
        console.error('Error keys:', Object.keys(contractorError))
        
        // Handle specific error cases
        if ((contractorError as any)?.code === 'PGRST116') {
          // No rows found - contractor with this slug doesn't exist
          console.log('No contractor found with slug:', contractorSlug)
          
          // If slug looks like a UUID, try to fetch by user_id as fallback
          const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
          console.log('🔍 Testing UUID regex on:', contractorSlug)
          console.log('🔍 UUID regex test result:', uuidRegex.test(contractorSlug))
          
          if (uuidRegex.test(contractorSlug)) {
            console.log('✅ Slug looks like a UUID, trying to fetch by user_id...')
            console.log('🔍 About to execute fallback query for user_id:', contractorSlug)
            
            try {
              const { data: contractorDataById, error: contractorErrorById } = await supabase
              .from('contractor_profiles')
              .select(`
                *,
                user:users!user_id (
              ...
              console.log('🔍 Fallback query completed. Data:', contractorDataById)
              console.log('🔍 Fallback query error:', contractorErrorById)

              if (contractorErrorById) {
                console.error('❌ Error fetching contractor by ID:', contractorErrorById)
                setError('Contractor not found')
                return
              }

              console.log('✅ Successfully fetched contractor by user_id:', contractorDataById)

            // If we found the contractor by user_id and they have a slug, redirect to the slug URL
            if (contractorDataById.slug) {
              console.log('🔄 Redirecting from UUID to slug URL:', `/profile/${contractorDataById.slug}`)
              window.location.href = `/profile/${contractorDataById.slug}`
              return
            }

            // Note: We don't check user.is_active for public profiles
            // Public contractor profiles should be visible regardless of account status

            setContractor(contractorDataById)
            ...
            const { data: reviewsData, error: reviewsError } = await supabase
              ...
              `)
              .eq('recipient', contractorDataById.user_id)
              .eq('is_verified', 'yes')
            ...
            console.log('❌ Slug does not look like a UUID, cannot use fallback')
          }
        }
      ...
      if (completedProjectsResponse.ok) {
        ...
        console.log('Completed projects API response:', completedProjectsData)
        
        // Filter and transform projects with photos and consent
        const projectsWithPhotos = completedProjectsData.reviews
          .filter((review: any) => {
            console.log('Review project photos:', review.project?.photos)
            return review.project?.photos?.before && review.project?.photos?.after
          })
          .map((review: any) => ({
        ...
        console.log('Filtered projects with photos:', projectsWithPhotos)
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">
            {error || 'The contractor profile you are looking for does not exist or may not be publicly available.'}
          </p>
          <div className="space-y-2 text-sm text-gray-500 mb-6">
            <p>This could happen if:</p>
            <ul className="list-disc list-inside space-y-1">
              <li>The contractor profile hasn't been created yet</li>
              <li>The profile is not publicly visible</li>
              <li>The URL slug is incorrect</li>
            </ul>
          </div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="space-y-12">
          ...
          {/* General Location */}
          {contractor.address && (contractor.address.city || contractor.address.province) && (
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-4">General Location</h2>
              <div className="flex items-center gap-2 text-gray-700">
                <MapPin className="h-5 w-5 text-gray-500" />
                <span>
                  {contractor.address.city && contractor.address.province 
                    ? `${contractor.address.city}, ${contractor.address.province}`
                    : contractor.address.city || contractor.address.province
                  }
                </span>
          ...
          {(contractor.insurance_general_liability || contractor.insurance_builders_risk || contractor.insurance_expiry) && (
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Insurance Coverage</h2>
              <div className="space-y-3">
                {contractor.insurance_general_liability && (
                  <div className="flex items-center justify-between py-2 border-b border-gray-100">
                    <span className="text-gray-600">General Liability</span>
                    <span className="font-semibold text-gray-900">
                      ${contractor.insurance_general_liability.toLocaleString()}
                    </span>
                  </div>
                )}
                {contractor.insurance_builders_risk && (
                  <div className="flex items-center justify-between py-2 border-b border-gray-100">
                    <span className="text-gray-600">Builders Risk</span>
                    <span className="font-semibold text-gray-900">
                      ${contractor.insurance_builders_risk.toLocaleString()}
                    </span>
                  </div>
                )}
          ...
          {/* Portfolio Files */}
          {contractor.portfolio_file && contractor.portfolio_file.length > 0 && (
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Portfolio Files</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {contractor.portfolio_file.map((file, index) => (
